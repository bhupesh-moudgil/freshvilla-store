 ...
 * 
 * Format: {type-prefix}-{state-code}-{city-code}-{store-number}
 * Example: br-dl-ndl-001 (Brand - Delhi - New Delhi - Store 001)
 *          int-mh-mum-001 (Integrated - Maharashtra - Mumbai - Store 001)
 * 
 * Type Prefixes:
...
/**
 * Generate store URL from city code, state code, store number, and type
 ...
 * @param {string} storeType - Store type ('brand' or 'integrated'), defaults to 'brand'
 * @returns {string} - Store URL (e.g., 'br-dl-ndl-001' or 'int-mh-mum-001')
 */
const generateStoreUrl = (stateCode, cityCode, storeNumber, storeType = 'brand') => {
  if (!stateCode || !cityCode || !storeNumber) {
    throw new Error('State code, city code, and store number are required');
  }

  const typePrefix = storeType === 'integrated' ? 'int' : 'br';
  const url = `${typePrefix}-${stateCode.toLowerCase()}-${cityCode.toLowerCase()}-${storeNumber}`;
  return url;
};
 ...
 * @param {object} storeData - Store data including city, state, cityCode, stateCode, storeType
 * @returns {Promise<object>} - Store data with generated storeNumber and storeUrl
 */
const generateStoreData = async (storeData) => {
  const { city, state, cityCode, stateCode, storeType = 'brand' } = storeData;
  ...
  // Generate store URL with storeType prefix
  const storeUrl = generateStoreUrl(stateCode, cityCode, storeNumber, storeType);
 ...
 * @param {string} storeUrl - Store URL (e.g., 'br-dl-ndl-001' or 'int-mh-mum-001')
 * @returns {object} - Parsed components { storeType, stateCode, cityCode, storeNumber }
 */
const parseStoreUrl = (storeUrl) => {
  const parts = storeUrl.split('-');
  
  if (parts.length !== 4) {
    throw new Error('Invalid store URL format. Expected format: type-state-city-number');
  }

  const storeType = parts[0] === 'int' ? 'integrated' : 'brand';

  return {
    storeType,
    stateCode: parts[1].toUpperCase(),
    cityCode: parts[2].toUpperCase(),
    storeNumber: parts[3]
  };
};
...
const isValidStoreUrl = (storeUrl) => {
  // Format: type-xx-xxx-nnn (br/int, dash, 2-3 letters, dash, 2-5 letters, dash, 3 digits)
  const urlPattern = /^(br|int)-[a-z]{2}-[a-z]{2,5}-\d{3}$/;
...
const getStoreDisplayInfo = async (storeUrl) => {
  ...
  const parsed = parseStoreUrl(store.storeUrl);
  
  return {
    id: store.id,
    displayName: store.name, // Full name: "FreshVilla New Delhi"
    shortName: `${store.city} ${store.storeNumber}`, // "New Delhi 001"
    url: store.storeUrl, // "br-dl-ndl-001" or "int-mh-mum-001"
    storeType: parsed.storeType, // "brand" or "integrated"
    typeLabel: parsed.storeType === 'brand' ? 'FreshVilla Brand Store' : 'Integrated Partner Store',
